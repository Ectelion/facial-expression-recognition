...
initTree <- function(trainingSet, formula = NULL) {
    # Training phase
    tree <- fitDTClassifier(trainingSet, formula)
    ...
    tree.crossValidation <- function(dataSet = trainingSet, K = 10) {
        crossValidation(dataSet, classifierType = "decision_tree", K)
    }
...
fitDTClassifier <- function(dataSet = loadData(), formula = NULL) {
    if (is.null(formula)) {
        ...
        # mouseChange   <- emotion ~ X116+X117+X118+X119+X120+X121+X122+X123+X124+X125+X126+X127
        ...
        # eyesChange    <- emotion ~ X36+X37+X38+X39+X40+X41+X42+X43+X44+X45+X46+X47
        ...
        optimalChange <- emotion ~ X52+X87+X89+X92+X103+X102+X101+X100+X117+X118+X120+X123+X125+X1+X23+X40+X24+X99+X38+X42+X45+X67+X88      
